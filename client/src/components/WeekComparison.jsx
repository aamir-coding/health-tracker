...
const METRICS = [
  ...
  { key:'sleepHours', label:'Sleep',  icon:Moon,       color:'purple', unit:'hrs',   better:'up',  fmt:v => v.toFixed(1) },
  { key:'waterMl',    label:'Water',  icon:Droplets,   color:'cyan',   unit:'ml',    better:'up',  fmt:v => Math.round(v).toLocaleString() },
  { key:'mood',       label:'Mood',   icon:Smile,      color:'green',  unit:'/5',    better:'up',  fmt:v => v.toFixed(1) },
  { key:'weight',     label:'Weight', icon:Scale,      color:'amber',  unit:'kg',    better:null,  fmt:v => v.toFixed(1) },
]
...
function Delta({ curr, prev, better }) {
  ...
  if (pct === 0)
    return (
      <span className="flex items-center gap-0.5 text-gray-400 dark:text-gray-500 text-xs">
        <Minus size={11} />0%
      </span>
    )
...
export default function WeekComparison() {
  ...
  const hasData = data && METRICS.some(m =>
    data.thisWeek?.[m.key] != null || data.lastWeek?.[m.key] != null
  )
    ...
    <div className="card p-5">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm" style={{ tableLayout: 'fixed' }}>
          ...
          <tbody>
            {METRICS.map(({ key, label, icon, color, unit, better, fmt }) => {
