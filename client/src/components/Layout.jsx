...
import { LayoutDashboard, SquarePen, ScrollText, Settings, Menu, X, LogOut, Heart, Sun, Moon, Monitor } from 'lucide-react'
...
function Avatar({ user, size = 'md' }) {
  ...
  if (user?.avatar) {
    return <img src={user.avatar} alt={user.name} className={`${sz} rounded-full object-cover flex-shrink-0`} />
  }
  const initials = user?.name?.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2) || '?'
  return (
    <div className={`${sz} bg-indigo-100 dark:bg-indigo-900/50 rounded-full flex items-center justify-center flex-shrink-0`}>
      <span className="text-indigo-700 dark:text-indigo-300 font-semibold">{initials}</span>
    </div>
  )
}
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-3 py-4 border-t border-gray-100 dark:border-gray-800">
        <Link to="/settings" onClick={onNavClick} className="flex items-center gap-3 px-3 py-2 mb-0.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
          <Avatar user={user} />
        ...
        <button
          ...
          className="flex items-center gap-3 w-full px-3 py-2.5 rounded-lg text-sm text-gray-500 dark:text-gray-400 hover:bg-red-50 dark:hover:bg-red-900/20 hover:text-red-600 dark:hover:text-red-400 transition-colors"
        >
          <LogOut size={16} />
          Sign out
        </button>
