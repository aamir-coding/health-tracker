...
export default function History() {
  ...
  const handleEdit = (log) => navigate('/log', { state: { log } })
    ...
    <Layout>
      ...
      {!loading && logs.length === 0 && (
        <div className="card p-12 text-center border-dashed">
          ...
          <p className="font-semibold text-gray-700">No logs yet</p>
                ...
                logs.map((log, i) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3.5 text-right tabular-nums">
                      {log.steps != null ? (
                        <span className="text-gray-700">{log.steps.toLocaleString()}</span>
                      ) : (
                    ...
                    <td className="px-4 py-3.5 text-right tabular-nums">
                      {log.sleepHours != null ? (
                        <span className="text-gray-700">{log.sleepHours}h</span>
                      ) : (
                        <span className="text-gray-300">—</span>
                      )}
                    </td>
                    <td className="px-4 py-3.5 text-right tabular-nums">
                      {log.waterMl != null ? (
                        <span className="text-gray-700">{log.waterMl}ml</span>
                      ) : (
                        <span className="text-gray-300">—</span>
                      )}
                    </td>
                    <td className="px-4 py-3.5 text-right tabular-nums">
                      {log.weight != null ? (
                        <span className="text-gray-700">{log.weight}kg</span>
                      ) : (
                        <span className="text-gray-300">—</span>
                      )}
                    </td>
                    <td className="px-4 py-3.5 text-center text-base">
                      {log.mood != null ? MOOD_EMOJI[log.mood] : <span className="text-gray-300 text-xs">—</span>}
                    </td>
          ...
          {logs.map((log) => (
            <div key={log._id} className="card p-4">
              ...
              <div className="flex flex-wrap gap-x-4 gap-y-1.5 text-xs text-gray-600">
                {log.steps != null && (
                  <span>🚶 <span className="tabular-nums">{log.steps.toLocaleString()}</span> steps</span>
                )}
                {log.sleepHours != null && (
                  <span>🌙 <span className="tabular-nums">{log.sleepHours}</span>h sleep</span>
                )}
                {log.waterMl != null && (
                  <span>💧 <span className="tabular-nums">{log.waterMl}</span>ml</span>
                )}
                {log.weight != null && (
                  <span>⚖️ <span className="tabular-nums">{log.weight}</span>kg</span>
                )}
                {log.mood != null && (
                  <span>{MOOD_EMOJI[log.mood]} Mood {log.mood}/5</span>
                )}
              </div>
              {log.notes && (
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button
              ...
              disabled={page === 1}
              className="btn-secondary gap-1 py-1.5 px-3"
            >
