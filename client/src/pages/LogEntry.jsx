...
export default function LogEntry() {
  ...
  const sanitise = (obj) => {
    ...
    const numFields = ['steps', 'sleepHours', 'waterMl', 'weight', 'mood']
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (existing) {
        await logsApi.update(existing._id, payload)
      } else {
      ...
      setError(
        err.response?.data?.errors?.[0]?.msg ||
          ...
          'Failed to save log. Please check your values and try again.'
      )
      setLoading(false)
    }
  }
      ...
      <div className="max-w-lg mx-auto">
        ...
        <div className="card p-6">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {success && (
              <div className="bg-green-50 border border-green-200 text-green-700 text-sm px-3.5 py-2.5 rounded-lg flex items-center gap-2">
                <Check size={16} />
            ...
            <Field label="😊 Mood">
              <div className="flex gap-2">
                {MOODS.map(({ value, emoji, label }) => (
                  <button
                    ...
                    type="button"
                    title={label}
                    onClick={() => set('mood', form.mood === value ? null : value)}
                    className={`flex-1 py-3 rounded-xl text-xl transition-all duration-150 border-2 ${
                      form.mood === value
                        ? 'border-indigo-400 bg-indigo-50 scale-110 shadow-sm shadow-indigo-100'
              ...
              {form.mood && (
                <p className="text-xs text-indigo-500 text-center mt-2 font-medium">
