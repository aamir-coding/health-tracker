...
function computeHealthScore(avg) {
  ...
  if (avg.sleepHours != null) {
    ...
    score += (s >= 7 && s <= 9 ? 25 : s >= 6 ? 18 : s >= 5 ? 10 : 5)
    max += 25
  }
...
export default function Dashboard() {
  ...
  const bmi = user?.height && latest?.weight
    ? (latest.weight / Math.pow(user.height / 100, 2)).toFixed(1)
    : null
  ...
  const metricCards = [
    ...
    { label: 'Water', value: latest?.waterMl ?? null, unit: 'ml', icon: '💧', sub: avg.waterMl != null ? `30d avg: ${Math.round(avg.waterMl)}ml` : null },
    ...
    <Layout>
      {loading ? <Skeleton /> : (
        <div className="space-y-5">
          ...
          {streak.streak > 0 && (
            <div className="card p-4 flex items-center gap-4 border-orange-200 dark:border-orange-800 bg-orange-50 dark:bg-orange-900/20">
              ...
              <div className="flex-1">
                ...
                <div className="text-orange-500 dark:text-orange-500 text-xs">Personal best: {streak.longest} days · keep logging daily to extend it</div>
              </div>
          ...
          {/* BMI + Health score */}
          {(bmi || healthScore != null || !user?.height) && (
              ...
              <div className="card p-5">
                ...
                {bmi ? (
                  <div className="mt-2">
                    ...
                    <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">Based on {latest?.weight}kg · {user.height}cm</p>
                  ...
                  <div className="mt-2">
                    ...
                    <div className="flex gap-2">
                      <input
                        type="number" placeholder="Height (cm)" min={50} max={300} step={1}
                        value={heightInput} onChange={e => setHeightInput(e.target.value)}
                        className="input-field"
                        onKeyDown={e => e.key === 'Enter' && handleSetHeight()}
                      />
                      <button onClick={handleSetHeight} disabled={settingHeight || !heightInput} className="btn-primary flex-shrink-0">
                        {settingHeight ? '…' : 'Save'}
                      </button>
                    </div>
                  </div>
                    ...
                    <div className="mt-3 h-2.5 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
                      <div
                        className={`h-full rounded-full transition-all duration-700 ${
                          healthScore >= 80 ? 'bg-green-500' : healthScore >= 60 ? 'bg-indigo-500' : healthScore >= 40 ? 'bg-amber-400' : 'bg-red-400'
                        }`}
